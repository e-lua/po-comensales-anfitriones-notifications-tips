...
func AddNotification_Service(notification models.Pg_Notifications) (int, bool, string, string) {

	notification.DateRegistered = time.Now()
	notification.WasView = false

	//Obtenemos las categorias
	error_update := notification_repository.Pg_Add(notification)
	if error_update != nil {
	...
	if notification.Priority == 1 && notification.TypeUser == 4 {
		...
		for _, iddeviceas_one := range get_respuesta.Data {

			//Buscamos que no se envien duplicados
			var iddevices_m []string
			iddevices_m = append(iddevices_m, iddeviceas_one)
			...
			if err != nil {
				log.Println("Error en la conexión con Firebase Cloud Message, detalles: " + err.Error())
			}
		}
...
func ShowNotification_Service(iduser_int int, limit_int int, page_int int, idtype_int int) (int, bool, string, []models.Pg_NotificationShow) {

	var notifications_trash []models.Pg_NotificationShow

	notifications, error_show := notification_repository.Pg_Find(iduser_int, limit_int, page_int, idtype_int)
	if error_show != nil {
	...
	error_update := notification_repository.Pg_Update(iduser_int, idtype_int)
	if error_update != nil {
