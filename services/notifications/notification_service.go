...
func UpdateNotifications_Service(iduser_int int, idtype_int int) (int, bool, string, string) {

	//Obtenemos las categorias
	error_update := notification_repository.Mo_Update(iduser_int, idtype_int)
	if error_update != nil {
		return 500, true, "Error en el servidor interno al intentar actualizar el estado de las notificaciones, detalles: " + error_update.Error(), ""
	}
